...
class Websocket(socket.socket):
    ...
    def wrap(self, data, mask):
        ...
        wrappedParts = []
        for part in parts[:-1]:
            wrappedParts.append(self._make_header(False, mask, len(part)) + part)
        wrappedParts.append(self._make_header(True, mask, len(parts[-1])) + parts[-1])
...
class Client(object):
    ...
    def update(self):
        ...
        if '\n' in self.buffer:
            ...
            for line in lines:
                self._interpret_line(line.strip())
